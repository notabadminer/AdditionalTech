...
public class ContainerPhotobioreactor extends Container {
	
		private TilePhotobioreactor tileEntity;
		private int lastWaterLevel;
		private int lastSlurryLevel;
		
		public ContainerPhotobioreactor(InventoryPlayer inventoryPlayer, TilePhotobioreactor tEntity) {
			tileEntity = tEntity;
			// the Slot constructor takes the IInventory and the slot number in that
			// it binds to
			// and the x-y coordinates it resides on-screen
			addSlotToContainer(new Slot(tileEntity, TilePhotobioreactor.slotInput, 18, 38));
			addSlotToContainer(new SlotOutput(tileEntity, TilePhotobioreactor.slotOutput, 143, 38));
	    ...
	    public void detectAndSendChanges()
	    {
	        ...
	        for (int i = 0; i < this.crafters.size(); ++i)
	        {
	            ...
	            {
	                icrafting.sendProgressBarUpdate(this, 0, this.tileEntity.waterLevel);
	            }
	            if (this.lastSlurryLevel != this.tileEntity.slurryLevel)
	            {
	                icrafting.sendProgressBarUpdate(this, 1, this.tileEntity.slurryLevel);
	            }
	        }
	    ...
	    public void updateProgressBar(int par1, int par2)
	    {
	        if (par1 == 0)
	        {
	            this.tileEntity.waterLevel = par2;
	        }
	        if (par1 == 1)
	        {
